...
module Elasticsearch6
  module Test
    class SearchSortTest < ::Elasticsearch6::Test::UnitTestCase
      subject { Elasticsearch6::DSL::Search::Sort.new }

      context "Search sort" do

        should "add a single field" do
          subject = Elasticsearch6::DSL::Search::Sort.new :foo
          assert_equal( [:foo], subject.to_hash )
        end

        should "add multiple fields" do
          subject = Elasticsearch6::DSL::Search::Sort.new [:foo, :bar]
          assert_equal( [:foo, :bar], subject.to_hash )
        end

        should "add a field with options" do
          subject = Elasticsearch6::DSL::Search::Sort.new foo: { order: 'desc', mode: 'avg' }
          assert_equal( [ { foo: { order: 'desc', mode: 'avg' } } ], subject.to_hash )
        end

        should "add fields with the DSL method" do
          subject = Elasticsearch6::DSL::Search::Sort.new do
            by :foo
            by :bar, order: 'desc'
          end
        ...
        should "not be empty" do
          subject = Elasticsearch6::DSL::Search::Sort.new foo: { order: 'desc' }
