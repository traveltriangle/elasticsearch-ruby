  ...
  module DSL
    module Search
      module Filters
        ...
        #     end
        #
        # @see http://elasticsearch6.org/guide/en/elasticsearch6/reference/current/query-dsl-has-parent-filter.html
        #
        class HasParent
          ...
          def query(*args, &block)
            @query = block ? @query = Elasticsearch6::DSL::Search::Query.new(*args, &block) : args.first
            self
          end
          ...
          def filter(*args, &block)
            @filter = block ? Elasticsearch6::DSL::Search::Filter.new(*args, &block) : args.first
            self
          end
