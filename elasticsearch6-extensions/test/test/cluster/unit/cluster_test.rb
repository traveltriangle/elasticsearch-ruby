...
class Elasticsearch6::Extensions::TestClusterTest < Elasticsearch6::Test::UnitTestCase
  ...
  context "The Test::Cluster" do
    context "module" do
      should "delegate the methods to the class" do
        ...
        Elasticsearch6::Extensions::Test::Cluster.start foo: 'bar'
        Elasticsearch6::Extensions::Test::Cluster.stop foo: 'bar'
        Elasticsearch6::Extensions::Test::Cluster.running? foo: 'bar'
        Elasticsearch6::Extensions::Test::Cluster.wait_for_green foo: 'bar'
      end
    end

    context "class" do
      setup do
        Elasticsearch6::Extensions::Test::Cluster::Cluster.any_instance.stubs(:__default_network_host).returns('_local_')

        @subject = Elasticsearch6::Extensions::Test::Cluster::Cluster.new(number_of_nodes: 1)
      ...
      should "have a default cluster name" do
        ...
        assert_equal 'elasticsearch6-test-foobar', Cluster::Cluster.new.__default_cluster_name
      end
      ...
      context "when stopping a cluster" do
        setup do
          @subject = Elasticsearch6::Extensions::Test::Cluster::Cluster.new
        end
      ...
      context "when determining a version" do
        setup do
          @subject = Elasticsearch6::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elasticsearch6'
        end

        should "return version from lib/elasticsearch6.X.Y.Z.jar" do
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['foo.jar', 'elasticsearch6-foo-1.0.0.jar', 'elasticsearch6-2.3.0.jar', 'elasticsearch6-bar-9.9.9.jar'])

          assert_equal '2.0', @subject.__determine_version
        end

        should "return version from `elasticsearch6 --version`" do
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(false)
          File.expects(:exist?).with('/foo/bar/bin/elasticsearch6').returns(true)
        ...
        should "return version from arguments" do
          cluster = Elasticsearch6::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elasticsearch6', version: '5.2'
        ...
        should "raise an exception when the version cannot be parsed from .jar" do
          ...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['elasticsearch6-100.jar'])
        ...
        should "raise an exception when the version cannot be converted to short version" do
          # There's no Elasticsearch6 version 3...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['elasticsearch6-3.2.1.jar'])
        ...
        should "raise an exception when the command cannot be found" do
          ...
          File.expects(:exist?).with('elasticsearch6').returns(false)
