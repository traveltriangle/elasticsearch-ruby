...
require 'elasticsearch6/extensions/backup'

class Elasticsearch6::Extensions::BackupTest < Elasticsearch6::Test::UnitTestCase
  context "The Backup gem extension" do
    setup do
      @model = stub trigger: true
      @subject = ::Backup::Database::Elasticsearch6.new(@model)
    end

    should "have a client" do
      assert_instance_of Elasticsearch6::Transport::Client, @subject.client
    end
    ...
    should "be configurable" do
      @subject = ::Backup::Database::Elasticsearch6.new(@model) do |db|
    ...
    should "raise an expection for an unsupported type of backup" do
      @subject = ::Backup::Database::Elasticsearch6.new(@model) { |db| db.mode = 'foobar' }
      assert_raise ::Backup::Database::Elasticsearch6::Error do
        @subject.perform!
      end
    end

    should "scan and scroll the index" do
      @subject = ::Backup::Database::Elasticsearch6.new(@model) { |db| db.indices = 'test' }
