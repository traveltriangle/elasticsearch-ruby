...
module Elasticsearch6
  module Extensions
    module Test
      ...
      #          assuming `elasticsearch6` is on $PATH.
      #
      ...
      #
      #      system 'wget https://artifacts.elastic.co/downloads/elasticsearch6/elasticsearch6-5.1.1.tar.gz'
      #      system 'tar -xvf elasticsearch6-5.1.1.tar.gz'
      #
      #      require 'elasticsearch6/extensions/test/cluster'
      #      Elasticsearch6::Extensions::Test::Cluster.start command: 'elasticsearch6-5.1.1/bin/elasticsearch6'
      #
      ...
      module Cluster
        ...
        class Cluster
          ...
          # @option arguments [String]  :command      Elasticsearch6 command (default: `elasticsearch6`)
          ...
          # @option arguments [Integer] :timeout_version Timeout when waiting for `elasticsearch6 --version` (default: 15)
          ...
          def initialize(arguments={})
            ...
            @arguments[:command]           ||= ENV.fetch('TEST_CLUSTER_COMMAND',   'elasticsearch6')
            ...
            @arguments[:path_logs]         ||= ENV.fetch('TEST_CLUSTER_LOGS',      '/tmp/log/elasticsearch6')
          ...
          #      Elasticsearch6::Extensions::Test::Cluster::Cluster.new(
          #        command: "/usr/local/Cellar/elasticsearch6/1.0.0.Beta2/bin/elasticsearch6"
          #      ).start
          #
          ...
          def __default_cluster_name
            "elasticsearch6-test-#{Socket.gethostname.downcase}"
          end
          ...
          # if not available, it parses the version number from the `lib/elasticsearch6-X.Y.Z.jar` file,
          # if that is not available, uses `elasticsearch6 --version` or `elasticsearch6 -v`
          #
          # @api private
          #
          # @return String
          ...
          def __determine_version
            ...
            version = if arguments[:version]
              arguments[:version]
            elsif File.exist?(path_to_lib) && !(jar = Dir.entries(path_to_lib).select { |f| f =~ /^elasticsearch6\-\d/ }.first).nil?
              __log "Determining version from [#{jar}]" if ENV['DEBUG']
              if m = jar.match(/elasticsearch6\-(\S+-)?(?<version>\d+\.\d+\.\d+).*/)
                m[:version]
              else
              ...
              if m = output.match(/Version: (\d\.\d.\d).*,/)
                ...
                raise RuntimeError, "Cannot determine Elasticsearch6 version from elasticsearch6 --version output [#{output}]"
              end
            end
