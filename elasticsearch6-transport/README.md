...
to an [Elasticsearch6](http://elasticsearch.org) cluster.
...
It does not handle calling the Elasticsearch6 API;
see the [`elasticsearch-api`](https://github.com/elasticsearch/elasticsearch-ruby/tree/master/elasticsearch-api) library.

The library is compatible with Ruby 1.9 or higher and with all versions of Elasticsearch6 since 0.90.
...
In the simplest form, connect to Elasticsearch6 running on <http://localhost:9200>
without any configuration:
    ...
    # => #<Elasticsearch6::Transport::Transport::Response:0x007fc5d506ce38 @status=200, @body={ ... } >
...
To connect to a specific Elasticsearch6 host:

    Elasticsearch6::Client.new host: 'search.myserver.com'

To connect to a host with specific port:

    Elasticsearch6::Client.new host: 'myhost:8080'

To connect to multiple hosts:

    Elasticsearch6::Client.new hosts: ['myhost1', 'myhost2']

Instead of Strings, you can pass host information as an array of Hashes:

    Elasticsearch6::Client.new hosts: [ { host: 'myhost1', port: 8080 }, { host: 'myhost2', port: 8080 } ]
...
Common URL parts -- scheme, HTTP authentication credentials, URL prefixes, etc -- are handled automatically:
    ...
    Elasticsearch6::Client.new url: 'https://username:password@api.server.org:4430/search'

You can pass multiple URLs separated by a comma:

    Elasticsearch6::Client.new urls: 'http://localhost:9200,http://localhost:9201'
...
You can pass the authentication credentials, scheme and port in the host configuration hash:
    ...
    Elasticsearch6::Client.new hosts: [
...
... or simply use the common URL format:
    ...
    Elasticsearch6::Client.new url: 'https://username:password@example.com:9200'
...
set the `log` argument:
    ...
    Elasticsearch6::Client.new log: true

To trace requests and responses in the _Curl_ format, set the `trace` argument:

    Elasticsearch6::Client.new trace: true
...
Or, you can use a custom {::Logger} instance:
    ...
    Elasticsearch6::Client.new logger: Logger.new(STDERR)
...
You can pass the client any conforming logger implementation:
    ...
    client = Elasticsearch6::Client.new logger: log

### Setting Timeouts

For many operations in Elasticsearch6, the default timeouts of HTTP libraries are too low.
To increase the timeout, you can use the `request_timeout` parameter:

    Elasticsearch6::Client.new request_timeout: 5*60
...
collection on initialization and reloading:
    ...
    Elasticsearch6::Client.new hosts: ['localhost:9200', 'localhost:9201'], randomize_hosts: true
...
on a different host:
    ...
    Elasticsearch6::Client.new hosts: ['localhost:9200', 'localhost:9201'], retry_on_failure: true

You can specify how many times should the client retry the request before it raises an exception
(the default is 3 times):

    Elasticsearch6::Client.new hosts: ['localhost:9200', 'localhost:9201'], retry_on_failure: 5

### Reloading Hosts

Elasticsearch6 by default dynamically discovers new nodes in the cluster. You can leverage this
...
on every 10,000th request:
    ...
    Elasticsearch6::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_connections: true

You can pass a specific number of requests after which the reloading should be performed:

    Elasticsearch6::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_connections: 1_000

To reload connections on failures, use:

    Elasticsearch6::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_on_failure: true

The reloading will timeout if not finished under 1 second by default. To change the setting:

    Elasticsearch6::Client.new hosts: ['localhost:9200', 'localhost:9201'], sniffer_timeout: 3
...
**NOTE:** When using reloading hosts ("sniffing") together with authentication, just pass the scheme,
    ...
    Elasticsearch6::Client.new host: 'localhost:9200',
...
{Elasticsearch6::Transport::Transport::Connections::Selector::RoundRobin} strategy.
...
Only when these would be unavailable, the strategy will use the other nodes:
    ...
    class RackIdSelector
      include Elasticsearch6::Transport::Transport::Connections::Selector::Base
    ...
    Elasticsearch6::Client.new hosts: ['x1.search.org', 'x2.search.org'], selector_class: RackIdSelector
...
To use a specific adapter for _Faraday_, pass it as the `adapter` argument:
    ...
    client = Elasticsearch6::Client.new adapter: :net_http_persistent
...
constructor, use the `transport_options` key:
    ...
    client = Elasticsearch6::Client.new transport_options: {
...
To configure the _Faraday_ instance directly, use a block:
    ...
    client = Elasticsearch6::Client.new(host: 'localhost', port: '9200') do |f|
      f.response :logger
      f.adapter  :typhoeus
    end

You can use any standard Faraday middleware and plugins in the configuration block,
for example sign the requests for the [AWS Elasticsearch6 service](https://aws.amazon.com/elasticsearch-service/):

    require 'faraday_middleware/aws_signers_v4'

    client = Elasticsearch6::Client.new url: 'https://search-my-cluster-abc123....es.amazonaws.com' do |f|
...
as the `transport` argument:
    ...
    #
    client = Elasticsearch6::Client.new transport: transport
...
Instead of passing the transport to the constructor, you can inject it at run time:
    ...
    faraday_client = Elasticsearch6::Transport::Transport::HTTP::Faraday.new \
...
You can also use a bundled [_Curb_](https://rubygems.org/gems/curb) based transport implementation:
    ...
    client = Elasticsearch6::Client.new transport_class: Elasticsearch6::Transport::Transport::HTTP::Curb
...
{Elasticsearch6::Transport::Transport::Base} module, implementing the required contract,
...
{Elasticsearch6::Transport::Transport::Serializer::Base} module, implementing the required contract,
...
The highest-level exception is {Elasticsearch6::Transport::Transport::Error}
and will be raised for any generic client *or* server errors.

{Elasticsearch6::Transport::Transport::ServerError} will be raised for server errors only.

As an example for response-specific errors, a `404` response status will raise
an {Elasticsearch6::Transport::Transport::Errors::NotFound} exception.

Finally, {Elasticsearch6::Transport::Transport::SnifferTimeoutError} will be raised
...
* {Elasticsearch6::Transport::Client} is composed of {Elasticsearch6::Transport::Transport}

* {Elasticsearch6::Transport::Transport} is composed of {Elasticsearch6::Transport::Transport::Connections},
...
* The {Elasticsearch6::Transport::Transport::Serializer::Base} implementations handle converting data for Elasticsearch6
  (eg. to JSON). You can implement your own serializer.

* {Elasticsearch6::Transport::Transport::Sniffer} allows to discover nodes in the cluster and use them as connections.

* {Elasticsearch6::Transport::Transport::Connections::Collection} is composed of
  {Elasticsearch6::Transport::Transport::Connections::Connection} instances and a selector instance.

* {Elasticsearch6::Transport::Transport::Connections::Connection} contains the connection attributes such as hostname and port,
  as well as the concrete persistent "session" connected to a specific node.

* The {Elasticsearch6::Transport::Transport::Connections::Selector::Base} implementations allow to choose connections
