  ...
  module API
    module Utils
      ...
      def __parse_java_source(path)
        ...
        prefix = "src/main/java/org/elasticsearch6/rest/action"
    ...
    #             --verbose \
    #             --crawl \
    #             --elasticsearch6=/path/to/elasticsearch6/source/code
    #
    # Features:
    #
    ...
    # * Search the <http://elasticsearch6.org> website to get proper documentation URLs
    ...
    class JsonGenerator < Thor
      ...
      method_option :elasticsearch6, default: __root.join('tmp/elasticsearch6'), desc: 'Path to directory with Elasticsearch6 source code'
      ...
      def generate
        ...
        rest_actions = Utils.__parse_java_source(options[:elasticsearch6].to_s)

        if rest_actions.empty?
          say_status 'ERROR', 'Cannot find Elasticsearch6 source in ' + options[:elasticsearch6].to_s, :red
          exit(1)
        end
        ...
        rest_actions.each do |name, info|
          ...
          if options[:crawl]
            begin
              response = RestClient.get "http://search.elasticsearch6.org/elastic-search-website/guide/_search?q=#{URI.escape(name.gsub(/\./, ' '))}"
              hits = JSON.load(response)['hits']['hits']
              if hit = hits.first
                if hit['_score'] > 0.2
                  doc_title = hit['fields']['title']
                  doc_url   = "http://elasticsearch6.org" + hit['fields']['url']
                end
              end
            rescue Exception => e
